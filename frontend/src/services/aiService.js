/**
 * Safar AI AI service — frontend entry point.
 *
...
if (import.meta.env.DEV) {
  console.info(
    '[Safar AI AI] Configuration status:',
...
function getClient() {
  ...
  if (!clientPromise) {
    clientPromise = import('groq-sdk')
      ...
      .catch((error) => {
        console.error('[Safar AI AI] Failed to load the Groq SDK:', error);
        return null;
      });
  }
...
const SYSTEM_PROMPT = `You are Safar AI, an expert AI travel assistant built by TravelCore. You help users plan detailed trip itineraries, discover destinations, explore local food and culture, suggest safe travel routes, and estimate travel budgets. Always be friendly, specific, and practical in your advice.
...
export async function generateAITravelResponse(message, options = {}) {
  ...
  if (apiUrl) {
    try {
      ...
      console.warn('[Safar AI AI] Railway API server call failed, falling back to local client:', error);
    }
  }
